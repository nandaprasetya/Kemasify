...
async function handleUpload(input) {
    ...
    try {
        ...
        try {
            ...
            statusEl.innerHTML = '<span style="color:var(--danger)">✗ Server error (HTTP ' + res.status + ')</span>';
            console.error('Upload non-JSON response:', rawText.substring(0, 300));
            return;
        }
        ...
        if (data.success) {
            ...
            statusEl.innerHTML = '<span style="color:var(--danger)">✗ ' + (data.message || 'Upload gagal') + '</span>';
        }
    } catch(e) {
...
async function generateAI() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<div class="spinner"></div> Membuat desain...';

    // Tampilkan status loading di panel
    ...
    try {
        ...
        // Baca sebagai teks dulu sebelum parse JSON
        const rawText = await res.text();
        let data;
        ...
        try {
            ...
            console.error('Response bukan JSON (HTTP ' + res.status + '):', rawText.substring(0, 800));
            showErrorStatus(
                res.status === 419 ? 'Sesi habis, silakan refresh halaman.' :
                res.status === 500 ? 'Server error (500). Cek storage/logs/laravel.log' :
                res.status === 404 ? 'Route tidak ditemukan. Jalankan: php artisan route:list | grep direct' :
                'HTTP ' + res.status + ' — response tidak valid. Cek console DevTools.'
            );
            return;
        }
        ...
        if (data.success) {
            ...
            const sidebarEl = document.getElementById('sidebar-token-balance');
            if (balEl     && data.token_balance !== undefined) balEl.textContent     = data.token_balance;
            if (sidebarEl && data.token_balance !== undefined) sidebarEl.textContent = data.token_balance;

            // Tampilkan gambar ke canvas dari base64 (lebih cepat dari URL)
            if (data.image_base64 && data.mime_type) {
                loadBase64ToCanvas('data:' + data.mime_type + ';base64,' + data.image_base64);
            } else if (data.image_url) {
                loadImageToCanvas(data.image_url);
            }

            showSuccessStatus('✓ Desain berhasil dibuat!');
        } else {
            showErrorStatus(data.error || 'Generate gagal.');
        }

    } catch (e) {
        console.error('Fetch error:', e);
        showErrorStatus('Koneksi gagal: ' + e.message);
    } finally {
        btn.disabled = false;
...
function showGeneratingStatus() {
    setAiStatus('processing', `
        <div style="display:flex;align-items:center;gap:8px;font-weight:600;color:var(--accent);">
            <div class="spinner"></div> Gemini sedang membuat desain...
        </div>
...
function showSuccessStatus(msg) {
    setAiStatus('completed', `<div style="font-weight:600;color:var(--accent);">${msg}</div>`);
}

function showErrorStatus(msg) {
    setAiStatus('failed', `
        <div style="font-weight:600;color:var(--danger);">✗ Gagal</div>
        <div class="text-sm" style="margin-top:4px;color:var(--text-muted);">${msg}</div>
    `);
}

function setAiStatus(type, html) {
    const existing = document.getElementById('ai-job-status');
    const newEl    = `<div class="job-status-card job-status-${type}" id="ai-job-status">${html}</div>`;
    const tab      = document.getElementById('tab-ai');
    if (existing) existing.outerHTML = newEl;
    else tab.insertAdjacentHTML('afterbegin', newEl);
}
...
function loadBase64ToCanvas(dataUrl) {
    const img = new Image();
    img.onload = () => {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        const scale = Math.min(canvas.width / img.width, canvas.height / img.height);
...
async function requestRender() {
    ...
    try {
        ...
        try {
            ...
            console.error('Render non-JSON response:', rawText.substring(0, 300));
            alert('Server error (HTTP ' + res.status + '). Cek console untuk detail.');
            btn.disabled = false;
